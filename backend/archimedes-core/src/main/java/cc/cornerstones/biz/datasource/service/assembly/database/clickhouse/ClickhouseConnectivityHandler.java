...
@Component
public class ClickhouseConnectivityHandler implements ConnectivityHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(ClickhouseConnectivityHandler.class);

    private static final String JDBC_DRIVER_PREFIX = "jdbc:clickhouse://";
    ...
    private static DruidDataSource getDataSource(ClickhouseConnectionProfile objectiveConnectionProfile) {
        String hashedKey =
    ...
    public DataSourceConnection createConnection(
        ...
        ClickhouseConnectionProfile objectiveConnectionProfile = null;
        try {
            objectiveConnectionProfile = JSONObject.toJavaObject(connectionProfile,
                    ClickhouseConnectionProfile.class);
        } catch (Exception e) {
            LOGGER.error("fail to transform clickhouse connection profile:{}", connectionProfile, e);
            throw new AbcResourceConflictException("unexpected clickhouse connection profile");
        }
        ...
        try {
            ...
            ClickhouseConnection objectiveConnection = new ClickhouseConnection();
            objectiveConnection.setConnection(connection);
            return objectiveConnection;
        } catch (SQLException e) {
            LOGGER.error("fail to create clickhouse connection for {}", connectionProfile, e);
            throw new AbcResourceConflictException("fail to create clickhouse connection");
        } finally {
        ...
        try {
            ClickHouseDataSource dataSource = new ClickHouseDataSource(url.toString(), properties);
            ...
            LOGGER.error("fail to create clickhouse connection for {}", url, e);
            throw new AbcResourceConflictException("fail to create clickhouse connection");
        } finally {
            // DO NOT close connection here
        }
    ...
    public void closeConnection(
        ...
        try {
            ...
            LOGGER.warn("fail to check clickhouse connection status", e);
        }

        try {
            objectiveConnection.getConnection().close();
        } catch (SQLException e) {
            LOGGER.error("fail to close clickhouse connection", e);

            throw new AbcResourceConflictException("fail to close clickhouse connection");
        }
    }
