...
@Component
public class OracleDmlHandler implements DmlHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(OracleDmlHandler.class);
    ...
    public QueryResult loadSampleDataOfDataTable(
        ...
        if (CollectionUtils.isEmpty(dataTableMetadata.getContextPath()) || dataTableMetadata.getContextPath().size() != 1) {
            return null;
        }
        ...
        sql.append("SELECT * FROM ")
                .append(AbcOracleUtils.buildEscapedName(databaseName)).append(".").append(AbcOracleUtils.buildEscapedName(dataTableMetadata.getName()))
    ...
    public ParseResult parseQuery(
            DataSourceConnection connection,
            String queryStatement) throws AbcUndefinedException {
        SQLStatement sqlStatement = SQLUtils.parseSingleStatement(queryStatement, DbType.oracle);
    ...
    public QueryResult testQuery(
        ...
        wrapperSql.append("SELECT * FROM (")
                ...
                .append(") ").append("ta" + System.currentTimeMillis())
    ...
    public QueryResult executeQuery(
        ...
        try (PreparedStatement statement = objectiveConnection.getConnection().prepareStatement(queryStatement)) {
    ...
    public QueryResult executeQuery(
            ...
            AbcPagination pagination) throws AbcUndefinedException {
        if (pagination != null) {
            String wrappedQueryStatement = queryBuilder.wrapWithPagination(queryStatement, pagination, "COMMON");
            return executeQuery(connection, wrappedQueryStatement);
        } else {
            return executeQuery(connection, queryStatement);
        }
    }
    ...
    public void executeQuery(
            JSONObject connectionProfile,
            String queryStatement,
            RowHandler rowHandler) throws AbcUndefinedException {
        DataSourceConnection connection = this.connectivityHandler.createConnection(connectionProfile);
        try {
