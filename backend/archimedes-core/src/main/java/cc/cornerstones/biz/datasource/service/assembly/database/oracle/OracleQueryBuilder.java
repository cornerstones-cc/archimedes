...
public class OracleQueryBuilder implements QueryBuilder {
    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(OracleQueryBuilder.class);
    ...
    public String buildQueryStatement(
        ...
        //
        String result = null;
        if (queryLayout.getPagination() == null) {
            result = sql.toString();
        } else {
            result = wrapWithPagination(sql.toString(), queryLayout.getPagination(), "COMMON");
        }
    ...
    private String buildGroupByClause(List<GroupByField> groupByFields) {
        ...
        groupByFields.forEach(groupByField -> {
            ...
            clause.append(AbcOracleUtils.buildEscapedName(groupByField.getFieldName()));
        });
    ...
    private String buildHavingClause(List<GroupFilter> groupFilters) {
        ...
        groupFilters.forEach(groupFilter -> {
            ...
            switch (groupFilter.getAggregateFunction()) {
                case MAX: {
                    clause.append("MAX(" + AbcOracleUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case MIN: {
                    clause.append("MIN(" + AbcOracleUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case SUM: {
                    clause.append("SUM(" + AbcOracleUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case VAR: {
                    clause.append("VAR_SAMP(" + AbcOracleUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case Varp: {
                    clause.append("VAR_POP(" + AbcOracleUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case COUNT: {
                    clause.append("COUNT(" + AbcOracleUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case STDDEV: {
                    clause.append("STDDEV_SAMP(" + AbcOracleUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case STDEVP: {
                    clause.append("STDDEV(" + AbcOracleUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case AVERAGE: {
                    clause.append("AVG(" + AbcOracleUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case PRODUCT: {
                    // TODO
    ...
    private String buildSelectCountFromClause(
        ...
        switch (tableType) {
            ...
            case DATABASE_VIEW: {
                ...
                clause.append(" ")
                        .append(AbcOracleUtils.buildEscapedName(databaseName))
                        .append(".")
                        .append(AbcOracleUtils.buildEscapedName(tableName));
            }
            break;
            case INDIRECT_TABLE: {
                ...
                for (int i = 0; i < selectionFields.size(); i++) {
                    ...
                    switch (selectionField.getType()) {
                        case PLAIN: {
                            ...
                            clause.append(AbcOracleUtils.buildEscapedName(fieldName));
                        }
                        break;
                        case EXPRESSION: {
                ...
                for (int i = 0; i < selectionFields.size(); i++) {
                    ...
                    switch (selectionField.getType()) {
                        case PLAIN: {
                            ...
                            clause.append(AbcOracleUtils.buildEscapedName(tableAlias))
                                    .append(".")
                                    .append(AbcOracleUtils.buildEscapedName(fieldName))
                                    .append(" AS ")
                                    .append(AbcOracleUtils.buildEscapedName(fieldName));
                        }
                        break;
    ...
    private String buildExpressionAggregateSelectionField(ExpressionAggregateSelectionField field) {
        switch (field.getAggregateFunction()) {
            case MAX: {
                return "MAX(" + AbcOracleUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
            }
            case MIN: {
                return "MIN(" + AbcOracleUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
            }
            case SUM: {
                switch (field.getSourceFieldType()) {
                    ...
                    case SMALLINT:
                    case MEDIUMINT:
                        return "SUM(cast(" + AbcOracleUtils.buildEscapedName(field.getSourceFieldName()) + " as int)" +
                                ")" + " " + "AS " + AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
                    case INT:
                        return "SUM(cast(" + AbcOracleUtils.buildEscapedName(field.getSourceFieldName()) + " as bigint)"
                                + ")" + " " + "AS " + AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
                    case LONG:
                    case DECIMAL:
                        return "SUM(" + AbcOracleUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                                AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
                    default:
                        return "''" + " AS " + AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
                }
            }
            case VAR: {
                return "VAR_SAMP(" + AbcOracleUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
            }
            case Varp: {
                return "VAR_POP(" + AbcOracleUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
            }
            case COUNT: {
                return "COUNT(" + AbcOracleUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
            }
            case STDDEV: {
                return "STDDEV_SAMP(" + AbcOracleUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
            }
            case STDEVP: {
                return "STDDEV(" + AbcOracleUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
            }
            case AVERAGE: {
                switch (field.getSourceFieldType()) {
                    ...
                    case SMALLINT:
                    case MEDIUMINT:
                        return "AVG(cast(" + AbcOracleUtils.buildEscapedName(field.getSourceFieldName()) + " as int)"
                                + ")" + " " + "AS "
                                + AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
                    case INT:
                        return "AVG(cast(" + AbcOracleUtils.buildEscapedName(field.getSourceFieldName()) + " as bigint)"
                                + ")" + " AS " +
                                AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
                    case LONG:
                    case DECIMAL:
                        return "AVG(" + AbcOracleUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                                AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
                    default:
                        return "''" + " AS " + AbcOracleUtils.buildEscapedName(field.getTargetFieldName());
                }
    ...
    private String buildNumberRangeParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcOracleUtils.buildEscapedName(parameterName))
                    .append(" = ")
    ...
    private String buildExactTextParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcOracleUtils.buildEscapedName(parameterName))
                    .append(" = ").append(AbcOracleUtils.surroundBySingleQuotes(AbcOracleUtils.buildEscapedValue(arrayOfParameterValue[0])));
        } else {
            clause.append(AbcOracleUtils.buildEscapedName(parameterName))
                    .append(" IN (");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
                if (i == 0) {
                    clause.append(AbcOracleUtils.surroundBySingleQuotes(AbcOracleUtils.buildEscapedValue(arrayOfParameterValue[i])));
                } else {
                    clause.append(",").append(AbcOracleUtils.surroundBySingleQuotes(AbcOracleUtils.buildEscapedValue(arrayOfParameterValue[i])));
                }
            }
            clause.append(")");
        }
    ...
    private String buildExactTextParameter(String parameterName, String parameterValue) {
        ...
        clause.append(AbcOracleUtils.buildEscapedName(parameterName))
                .append(" = ").append(AbcOracleUtils.surroundBySingleQuotes(AbcOracleUtils.buildEscapedValue(parameterValue)));
        ...
        if (arrayOfParameterValue.length == 1) {
            ...
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
                if (i == 0) {
                    clause.append(AbcOracleUtils.buildEscapedName(parameterName))
                            .append(" LIKE ")
                            .append(AbcOracleUtils.surroundBySingleQuotes("%" + AbcOracleUtils.buildEscapedValue(arrayOfParameterValue[i]) + "%"));
                } else {
                    clause.append(" OR ")
                            .append(AbcOracleUtils.buildEscapedName(parameterName))
                            .append(" LIKE ")
                            .append(AbcOracleUtils.surroundBySingleQuotes("%" + AbcOracleUtils.buildEscapedValue(arrayOfParameterValue[i]) + "%"));
                }
            }
    ...
    private String buildLeftFuzzyTextParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcOracleUtils.buildEscapedName(parameterName))
                    .append(" LIKE ")
                    .append("'%")
                    .append(AbcOracleUtils.buildEscapedValue(arrayOfParameterValue[0]))
                    .append("'");
        } else {
            clause.append("(");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
    ...
    private String buildRightFuzzyTextParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcOracleUtils.buildEscapedName(parameterName))
                    .append(" LIKE ")
                    .append(AbcOracleUtils.surroundBySingleQuotes(AbcOracleUtils.buildEscapedValue(arrayOfParameterValue[0]) + "%"));
        } else {
            clause.append("(");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
                if (i == 0) {
    ...
    private String buildDateRangeParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcOracleUtils.buildEscapedName(parameterName))
                    .append(" = ")
                    .append(AbcOracleUtils.surroundBySingleQuotes(arrayOfParameterValue[0]));
        } else if (arrayOfParameterValue.length == 2) {
            clause.append(AbcOracleUtils.buildEscapedName(parameterName))
                    .append(" BETWEEN ")
                    .append(AbcOracleUtils.surroundBySingleQuotes(arrayOfParameterValue[0] + " 00:00:00"))
                    .append(" AND ")
                    .append(AbcOracleUtils.surroundBySingleQuotes(arrayOfParameterValue[1] + " 23:59:59"));
        } else {
            clause.append(AbcOracleUtils.buildEscapedName(parameterName))
                    .append(" IN (");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
                if (i == 0) {
                    clause.append(AbcOracleUtils.surroundBySingleQuotes(arrayOfParameterValue[i]));
                } else {
                    clause.append(",").append(AbcOracleUtils.surroundBySingleQuotes(arrayOfParameterValue[i]));
                }
            }
            clause.append(")");
        }
    ...
    private String buildTimeRangeParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            ...
            clause.append(AbcOracleUtils.buildEscapedName(parameterName))
                    ...
                    .append(AbcOracleUtils.surroundBySingleQuotes(arrayOfParameterValue[0]))
                    .append(" AND ")
                    .append(AbcOracleUtils.surroundBySingleQuotes(arrayOfParameterValue[1]));
        } else {
            clause.append(AbcOracleUtils.buildEscapedName(parameterName))
                    .append(" IN (");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
    ...
    private String buildSortClause(AbcSort sort) {
        ...
        for (AbcOrder order : sort.getOrders()) {
            if (clause.length() > 0) {
                clause.append(",")
                        .append(AbcOracleUtils.buildEscapedName(order.getProperty()))
                        .append(" ")
                        .append(order.getDirection());
            } else {
                clause.append(AbcOracleUtils.buildEscapedName(order.getProperty()))
                        .append(" ")
                        .append(order.getDirection());
            }
        }
    ...
    public String wrapWithPagination(
            String querySqlWithoutPagination,
            AbcPagination pagination,
        ...
        switch (oracleVersion) {
            case "ORACLE_12C":
                clause.append(querySqlWithoutPagination)
                        .append(" offset " + pagination.getPage() * pagination.getSize())
                        .append(" rows fetch next " + pagination.getSize() + " rows only");
                break;
            case "ORACLE_11G":
            case "COMMON":
            default:
                clause.append("SELECT * FROM")
                        .append(" (")
                        .append("SELECT ROWNUM AS rowno, sub_t1.* FROM")
                        .append(" (")
                        .append(querySqlWithoutPagination)
                        .append(") sub_t1")
                        .append(" WHERE ROWNUM <= " + pagination.getSize() * (pagination.getPage() + 1))
                        .append(") sub_t2")
                        .append(" WHERE sub_t2.rowno > " + pagination.getSize() * pagination.getPage());
                break;
        }
