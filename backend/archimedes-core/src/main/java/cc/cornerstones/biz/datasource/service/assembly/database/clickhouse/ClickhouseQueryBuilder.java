...
public class ClickhouseQueryBuilder implements QueryBuilder {
    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(ClickhouseQueryBuilder.class);
    ...
    public void verifyStatement(String statement) throws AbcUndefinedException {
        ...
        ClickhouseWallProvider provider = new ClickhouseWallProvider();
    ...
    @Override
    public String buildCountStatement(DataSourceQueryDto queryLayout) throws AbcUndefinedException {
    ...
    private String buildGroupByClause(List<GroupByField> groupByFields) {
        ...
        groupByFields.forEach(groupByField -> {
            ...
            clause.append(AbcClickhouseUtils.buildEscapedName(groupByField.getFieldName()));
        });
    ...
    private String buildHavingClause(List<GroupFilter> groupFilters) {
        ...
        groupFilters.forEach(groupFilter -> {
            ...
            switch (groupFilter.getAggregateFunction()) {
                case MAX: {
                    clause.append("MAX(" + AbcClickhouseUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case MIN: {
                    clause.append("MIN(" + AbcClickhouseUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case SUM: {
                    clause.append("SUM(" + AbcClickhouseUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case VAR: {
                    clause.append("VAR_SAMP(" + AbcClickhouseUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case Varp: {
                    clause.append("VAR_POP(" + AbcClickhouseUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case COUNT: {
                    clause.append("COUNT(" + AbcClickhouseUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case STDDEV: {
                    clause.append("STDDEV_SAMP(" + AbcClickhouseUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case STDEVP: {
                    clause.append("STDDEV(" + AbcClickhouseUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case AVERAGE: {
                    clause.append("AVG(" + AbcClickhouseUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case PRODUCT: {
                    // TODO
    ...
    private String buildSelectCountFromClause(
        ...
        switch (tableType) {
            ...
            case DATABASE_VIEW: {
                ...
                clause.append(" ")
                        .append(AbcClickhouseUtils.buildEscapedName(databaseName))
                        .append(".")
                        .append(AbcClickhouseUtils.buildEscapedName(tableName));
            }
            break;
            case INDIRECT_TABLE: {
                ...
                for (int i = 0; i < selectionFields.size(); i++) {
                    ...
                    switch (selectionField.getType()) {
                        case PLAIN: {
                            ...
                            clause.append(AbcClickhouseUtils.buildEscapedName(fieldName));
                        }
                        break;
                        case EXPRESSION: {
                ...
                for (int i = 0; i < selectionFields.size(); i++) {
                    ...
                    switch (selectionField.getType()) {
                        case PLAIN: {
                            ...
                            clause.append(AbcClickhouseUtils.buildEscapedName(tableAlias))
                                    .append(".")
                                    .append(AbcClickhouseUtils.buildEscapedName(fieldName))
                                    .append(" AS ")
                                    .append(AbcClickhouseUtils.buildEscapedName(fieldName));
                        }
                        break;
    ...
    private String buildExpressionAggregateSelectionField(ExpressionAggregateSelectionField field) {
        switch (field.getAggregateFunction()) {
            case MAX: {
                return "MAX(" + AbcClickhouseUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
            }
            case MIN: {
                return "MIN(" + AbcClickhouseUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
            }
            case SUM: {
                switch (field.getSourceFieldType()) {
                    ...
                    case SMALLINT:
                    case MEDIUMINT:
                        return "SUM(cast(" + AbcClickhouseUtils.buildEscapedName(field.getSourceFieldName()) + " as int)" +
                                ")" + " " + "AS " + AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
                    case INT:
                        return "SUM(cast(" + AbcClickhouseUtils.buildEscapedName(field.getSourceFieldName()) + " as bigint)"
                                + ")" + " " + "AS " + AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
                    case LONG:
                    case DECIMAL:
                        return "SUM(" + AbcClickhouseUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                                AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
                    default:
                        return "''" + " AS " + AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
                }
            }
            case VAR: {
                return "VAR_SAMP(" + AbcClickhouseUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
            }
            case Varp: {
                return "VAR_POP(" + AbcClickhouseUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
            }
            case COUNT: {
                return "COUNT(" + AbcClickhouseUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
            }
            case STDDEV: {
                return "STDDEV_SAMP(" + AbcClickhouseUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
            }
            case STDEVP: {
                return "STDDEV(" + AbcClickhouseUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
            }
            case AVERAGE: {
                switch (field.getSourceFieldType()) {
                    ...
                    case SMALLINT:
                    case MEDIUMINT:
                        return "AVG(cast(" + AbcClickhouseUtils.buildEscapedName(field.getSourceFieldName()) + " as int)"
                                + ")" + " " + "AS "
                                + AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
                    case INT:
                        return "AVG(cast(" + AbcClickhouseUtils.buildEscapedName(field.getSourceFieldName()) + " as bigint)"
                                + ")" + " AS " +
                                AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
                    case LONG:
                    case DECIMAL:
                        return "AVG(" + AbcClickhouseUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                                AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
                    default:
                        return "''" + " AS " + AbcClickhouseUtils.buildEscapedName(field.getTargetFieldName());
                }
    ...
    private String buildNumberRangeParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcClickhouseUtils.buildEscapedName(parameterName))
                    .append(" = ")
    ...
    private String buildExactTextParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcClickhouseUtils.buildEscapedName(parameterName))
                    .append(" = ").append(AbcClickhouseUtils.surroundBySingleQuotes(AbcClickhouseUtils.buildEscapedValue(arrayOfParameterValue[0])));
        } else {
            clause.append(AbcClickhouseUtils.buildEscapedName(parameterName))
                    .append(" IN (");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
                if (i == 0) {
                    clause.append(AbcClickhouseUtils.surroundBySingleQuotes(AbcClickhouseUtils.buildEscapedValue(arrayOfParameterValue[i])));
                } else {
                    clause.append(",").append(AbcClickhouseUtils.surroundBySingleQuotes(AbcClickhouseUtils.buildEscapedValue(arrayOfParameterValue[i])));
                }
            }
            clause.append(")");
        }
    ...
    private String buildExactTextParameter(String parameterName, String parameterValue) {
        ...
        clause.append(AbcClickhouseUtils.buildEscapedName(parameterName))
                .append(" = ").append(AbcClickhouseUtils.surroundBySingleQuotes(AbcClickhouseUtils.buildEscapedValue(parameterValue)));
        ...
        if (arrayOfParameterValue.length == 1) {
            ...
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
                if (i == 0) {
                    clause.append(AbcClickhouseUtils.buildEscapedName(parameterName))
                            .append(" LIKE ")
                            .append(AbcClickhouseUtils.surroundBySingleQuotes("%" + AbcClickhouseUtils.buildEscapedValue(arrayOfParameterValue[i]) + "%"));
                } else {
                    clause.append(" OR ")
                            .append(AbcClickhouseUtils.buildEscapedName(parameterName))
                            .append(" LIKE ")
                            .append(AbcClickhouseUtils.surroundBySingleQuotes("%" + AbcClickhouseUtils.buildEscapedValue(arrayOfParameterValue[i]) + "%"));
                }
            }
    ...
    private String buildLeftFuzzyTextParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcClickhouseUtils.buildEscapedName(parameterName))
                    .append(" LIKE ")
                    .append("'%")
                    .append(AbcClickhouseUtils.buildEscapedValue(arrayOfParameterValue[0]))
                    .append("'");
        } else {
            clause.append("(");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
    ...
    private String buildRightFuzzyTextParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcClickhouseUtils.buildEscapedName(parameterName))
                    .append(" LIKE ")
                    .append(AbcClickhouseUtils.surroundBySingleQuotes(AbcClickhouseUtils.buildEscapedValue(arrayOfParameterValue[0]) + "%"));
        } else {
            clause.append("(");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
                if (i == 0) {
    ...
    private String buildDateRangeParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcClickhouseUtils.buildEscapedName(parameterName))
                    .append(" = ")
                    .append(AbcClickhouseUtils.surroundBySingleQuotes(arrayOfParameterValue[0]));
        } else if (arrayOfParameterValue.length == 2) {
            clause.append(AbcClickhouseUtils.buildEscapedName(parameterName))
                    .append(" BETWEEN ")
                    .append(AbcClickhouseUtils.surroundBySingleQuotes(arrayOfParameterValue[0] + " 00:00:00"))
                    .append(" AND ")
                    .append(AbcClickhouseUtils.surroundBySingleQuotes(arrayOfParameterValue[1] + " 23:59:59"));
        } else {
            clause.append(AbcClickhouseUtils.buildEscapedName(parameterName))
                    .append(" IN (");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
                if (i == 0) {
                    clause.append(AbcClickhouseUtils.surroundBySingleQuotes(arrayOfParameterValue[i]));
                } else {
                    clause.append(",").append(AbcClickhouseUtils.surroundBySingleQuotes(arrayOfParameterValue[i]));
                }
            }
            clause.append(")");
        }
    ...
    private String buildTimeRangeParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            ...
            clause.append(AbcClickhouseUtils.buildEscapedName(parameterName))
                    ...
                    .append(AbcClickhouseUtils.surroundBySingleQuotes(arrayOfParameterValue[0]))
                    .append(" AND ")
                    .append(AbcClickhouseUtils.surroundBySingleQuotes(arrayOfParameterValue[1]));
        } else {
            clause.append(AbcClickhouseUtils.buildEscapedName(parameterName))
                    .append(" IN (");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
    ...
    private String buildSortClause(AbcSort sort) {
        ...
        for (AbcOrder order : sort.getOrders()) {
            if (clause.length() > 0) {
                clause.append(",")
                        .append(AbcClickhouseUtils.buildEscapedName(order.getProperty()))
                        .append(" ")
                        .append(order.getDirection());
            } else {
                clause.append(AbcClickhouseUtils.buildEscapedName(order.getProperty()))
                        .append(" ")
                        .append(order.getDirection());
            }
        }
