...
public class PostgresqlQueryBuilder implements QueryBuilder {
    ...
    private String buildGroupByClause(List<GroupByField> groupByFields) {
        ...
        groupByFields.forEach(groupByField -> {
            ...
            clause.append(AbcPostgresqlUtils.buildEscapedName(groupByField.getFieldName()));
        });
    ...
    private String buildHavingClause(List<GroupFilter> groupFilters) {
        ...
        groupFilters.forEach(groupFilter -> {
            ...
            switch (groupFilter.getAggregateFunction()) {
                case MAX: {
                    clause.append("MAX(" + AbcPostgresqlUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case MIN: {
                    clause.append("MIN(" + AbcPostgresqlUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case SUM: {
                    clause.append("SUM(" + AbcPostgresqlUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case VAR: {
                    clause.append("VAR_SAMP(" + AbcPostgresqlUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case Varp: {
                    clause.append("VAR_POP(" + AbcPostgresqlUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case COUNT: {
                    clause.append("COUNT(" + AbcPostgresqlUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case STDDEV: {
                    clause.append("STDDEV_SAMP(" + AbcPostgresqlUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case STDEVP: {
                    clause.append("STDDEV(" + AbcPostgresqlUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case AVERAGE: {
                    clause.append("AVG(" + AbcPostgresqlUtils.buildEscapedName(groupFilter.getFieldName()) + ")");
                }
                break;
                case PRODUCT: {
                    // TODO
    ...
    private String buildSelectCountFromClause(
        ...
        switch (tableType) {
            ...
            case DATABASE_VIEW: {
                ...
                clause.append(" ")
                        .append(AbcPostgresqlUtils.buildEscapedName(databaseName))
                        .append(".")
                        .append(AbcPostgresqlUtils.buildEscapedName(tableName));
            }
            break;
            case INDIRECT_TABLE: {
                ...
                for (int i = 0; i < selectionFields.size(); i++) {
                    ...
                    switch (selectionField.getType()) {
                        case PLAIN: {
                            ...
                            clause.append(AbcPostgresqlUtils.buildEscapedName(fieldName));
                        }
                        break;
                        case EXPRESSION: {
                ...
                for (int i = 0; i < selectionFields.size(); i++) {
                    ...
                    switch (selectionField.getType()) {
                        case PLAIN: {
                            ...
                            clause.append(AbcPostgresqlUtils.buildEscapedName(tableAlias))
                                    .append(".")
                                    .append(AbcPostgresqlUtils.buildEscapedName(fieldName))
                                    .append(" AS ")
                                    .append(AbcPostgresqlUtils.buildEscapedName(fieldName));
                        }
                        break;
    ...
    private String buildExpressionAggregateSelectionField(ExpressionAggregateSelectionField field) {
        switch (field.getAggregateFunction()) {
            case MAX: {
                return "MAX(" + AbcPostgresqlUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
            }
            case MIN: {
                return "MIN(" + AbcPostgresqlUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
            }
            case SUM: {
                switch (field.getSourceFieldType()) {
                    ...
                    case SMALLINT:
                    case MEDIUMINT:
                        return "SUM(cast(" + AbcPostgresqlUtils.buildEscapedName(field.getSourceFieldName()) + " as int)" +
                                ")" + " " + "AS " + AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
                    case INT:
                        return "SUM(cast(" + AbcPostgresqlUtils.buildEscapedName(field.getSourceFieldName()) + " as bigint)"
                                + ")" + " " + "AS " + AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
                    case LONG:
                    case DECIMAL:
                        return "SUM(" + AbcPostgresqlUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                                AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
                    default:
                        return "''" + " AS " + AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
                }
            }
            case VAR: {
                return "VAR_SAMP(" + AbcPostgresqlUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
            }
            case Varp: {
                return "VAR_POP(" + AbcPostgresqlUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
            }
            case COUNT: {
                return "COUNT(" + AbcPostgresqlUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
            }
            case STDDEV: {
                return "STDDEV_SAMP(" + AbcPostgresqlUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
            }
            case STDEVP: {
                return "STDDEV(" + AbcPostgresqlUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                        AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
            }
            case AVERAGE: {
                switch (field.getSourceFieldType()) {
                    ...
                    case SMALLINT:
                    case MEDIUMINT:
                        return "AVG(cast(" + AbcPostgresqlUtils.buildEscapedName(field.getSourceFieldName()) + " as int)"
                                + ")" + " " + "AS "
                                + AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
                    case INT:
                        return "AVG(cast(" + AbcPostgresqlUtils.buildEscapedName(field.getSourceFieldName()) + " as bigint)"
                                + ")" + " AS " +
                                AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
                    case LONG:
                    case DECIMAL:
                        return "AVG(" + AbcPostgresqlUtils.buildEscapedName(field.getSourceFieldName()) + ")" + " AS " +
                                AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
                    default:
                        return "''" + " AS " + AbcPostgresqlUtils.buildEscapedName(field.getTargetFieldName());
                }
    ...
    private String buildNumberRangeParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcPostgresqlUtils.buildEscapedName(parameterName))
                    .append(" = ")
    ...
    private String buildExactTextParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcPostgresqlUtils.buildEscapedName(parameterName))
                    .append(" = ").append(AbcPostgresqlUtils.surroundBySingleQuotes(AbcPostgresqlUtils.buildEscapedValue(arrayOfParameterValue[0])));
        } else {
            clause.append(AbcPostgresqlUtils.buildEscapedName(parameterName))
                    .append(" IN (");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
                if (i == 0) {
                    clause.append(AbcPostgresqlUtils.surroundBySingleQuotes(AbcPostgresqlUtils.buildEscapedValue(arrayOfParameterValue[i])));
                } else {
                    clause.append(",").append(AbcPostgresqlUtils.surroundBySingleQuotes(AbcPostgresqlUtils.buildEscapedValue(arrayOfParameterValue[i])));
                }
            }
            clause.append(")");
        }
    ...
    private String buildExactTextParameter(String parameterName, String parameterValue) {
        ...
        clause.append(AbcPostgresqlUtils.buildEscapedName(parameterName))
                .append(" = ").append(AbcPostgresqlUtils.surroundBySingleQuotes(AbcPostgresqlUtils.buildEscapedValue(parameterValue)));
    ...
    private String buildFullFuzzyTextParameter(String parameterName, String[] arrayOfParameterValue) {
        StringBuilder clause = new StringBuilder();
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcPostgresqlUtils.buildEscapedName(parameterName)).append("::text")
                    .append(" LIKE ")
                    .append("'%")
                    .append(arrayOfParameterValue[0])
                    .append("%'");
            ...
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
                if (i == 0) {
                    clause.append(AbcPostgresqlUtils.buildEscapedName(parameterName)).append("::text")
                            .append(" LIKE ")
                            .append(AbcPostgresqlUtils.surroundBySingleQuotes("%" + AbcPostgresqlUtils.buildEscapedValue(arrayOfParameterValue[i]) + "%"));
                } else {
                    clause.append(" OR ")
                            .append(AbcPostgresqlUtils.buildEscapedName(parameterName)).append("::text")
                            .append(" LIKE ")
                            .append(AbcPostgresqlUtils.surroundBySingleQuotes("%" + AbcPostgresqlUtils.buildEscapedValue(arrayOfParameterValue[i]) + "%"));
                }
            }
    ...
    private String buildLeftFuzzyTextParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcPostgresqlUtils.buildEscapedName(parameterName)).append("::text")
                    .append(" LIKE ")
                    .append("'%")
                    .append(AbcPostgresqlUtils.buildEscapedValue(arrayOfParameterValue[0]))
                    .append("'");
        } else {
            clause.append("(");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
    ...
    private String buildRightFuzzyTextParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcPostgresqlUtils.buildEscapedName(parameterName)).append("::text")
                    .append(" LIKE ")
                    .append(AbcPostgresqlUtils.surroundBySingleQuotes(AbcPostgresqlUtils.buildEscapedValue(arrayOfParameterValue[0]) + "%"));
        } else {
            clause.append("(");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
                if (i == 0) {
    ...
    private String buildDateRangeParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            clause.append(AbcPostgresqlUtils.buildEscapedName(parameterName))
                    .append(" = ")
                    .append(AbcPostgresqlUtils.surroundBySingleQuotes(arrayOfParameterValue[0]));
        } else if (arrayOfParameterValue.length == 2) {
            clause.append(AbcPostgresqlUtils.buildEscapedName(parameterName))
                    .append(" BETWEEN ")
                    .append(AbcPostgresqlUtils.surroundBySingleQuotes(arrayOfParameterValue[0] + " 00:00:00"))
                    .append(" AND ")
                    .append(AbcPostgresqlUtils.surroundBySingleQuotes(arrayOfParameterValue[1] + " 23:59:59"));
        } else {
            clause.append(AbcPostgresqlUtils.buildEscapedName(parameterName))
                    .append(" IN (");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
                if (i == 0) {
                    clause.append(AbcPostgresqlUtils.surroundBySingleQuotes(arrayOfParameterValue[i]));
                } else {
                    clause.append(",").append(AbcPostgresqlUtils.surroundBySingleQuotes(arrayOfParameterValue[i]));
                }
            }
            clause.append(")");
        }
    ...
    private String buildTimeRangeParameter(String parameterName, String[] arrayOfParameterValue) {
        ...
        if (arrayOfParameterValue.length == 1) {
            ...
            clause.append(AbcPostgresqlUtils.buildEscapedName(parameterName))
                    ...
                    .append(AbcPostgresqlUtils.surroundBySingleQuotes(arrayOfParameterValue[0]))
                    .append(" AND ")
                    .append(AbcPostgresqlUtils.surroundBySingleQuotes(arrayOfParameterValue[1]));
        } else {
            clause.append(AbcPostgresqlUtils.buildEscapedName(parameterName))
                    .append(" IN (");
            for (int i = 0; i < arrayOfParameterValue.length; i++) {
    ...
    private String buildSortClause(AbcSort sort) {
        ...
        for (AbcOrder order : sort.getOrders()) {
            if (clause.length() > 0) {
                clause.append(",")
                        .append(AbcPostgresqlUtils.buildEscapedName(order.getProperty()))
                        .append(" ")
                        .append(order.getDirection());
            } else {
                clause.append(AbcPostgresqlUtils.buildEscapedName(order.getProperty()))
                        .append(" ")
                        .append(order.getDirection());
            }
        }
    ...
    private String buildPaginationClause(AbcPagination pagination) {
        ...
        StringBuilder clause = new StringBuilder();
        clause.append(pagination.getSize())
                .append(" OFFSET ")
                .append(pagination.getPage() * pagination.getSize());
