...
import static cc.cornerstones.biz.administration.systemsettings.service.impl.SystemSettingsServiceImpl.MAXIMUM_NUMBER_OF_FILES_EXPORTED_BY_ONE_EXPORT_TASK;
import static cc.cornerstones.biz.administration.systemsettings.service.impl.SystemSettingsServiceImpl.MAXIMUM_NUMBER_OF_IMAGES_EXPORTED_BY_ONE_EXPORT_TASK;

@Component
public class ExportCsvWithAttachmentsTaskHandler extends ExportTaskHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(ExportCsvWithAttachmentsTaskHandler.class);
    ...
    public static final String TASK_TYPE_EXPORT = "export_csv_w_attachments";
    public static final String TASK_HANDLER_NAME_EXPORT = "export_csv_w_attachments";
    ...
    public void execute(
        ...
        ExportCsvWithAttachmentsPayload input = JSONObject.toJavaObject(payload,
                ExportCsvWithAttachmentsPayload.class);
        ...
        exportTaskDo.setExportOption(ExportOptionEnum.EXPORT_CSV_W_ATTACHMENTS);
    ...
    private void export(
        ...
        String dataFileName = String.format("%s.csv", taskName);
        ...
        Path fileAttachmentsDirectoryPath = Paths.get(this.projectExportPath, mainDirectoryName, fileAttachmentsDirectoryName);
        ...
        try {
            fileOutputStream = new FileOutputStream(dataFilePath.toFile());
            ...
            LOGGER.error("failed to create file:{}", dataFilePath, e);
            ...
            if (outputStreamWriter != null) {
                try {
                    ...
                    LOGGER.warn("failed to close file:{}", dataFilePath, e2);
                }
            }
        ...
        if (shouldCancel) {
            if (outputStreamWriter != null) {
                try {
                    ...
                    LOGGER.warn("failed to close " + dataFilePath + ". " + e.getMessage(), e);
                }
            }
                ...
                public Integer process(List<List<Object>> rows, List<String> columnLabels) throws AbcUndefinedException {
                    ...
                    for (int rowNo = 0; rowNo < rows.size(); rowNo++) {
                        ...
                        for (int columnNo = 0; columnNo < columnLabels.size(); columnNo++) {
                            ...
                            Object rawColumnValue = row.get(columnNo);

                            Object transformedColumnValue = rawColumnValue;

                            if (rawColumnValue != null && (rawColumnValue instanceof String)) {
                                String columnValueString = (String) rawColumnValue;
                                ...
                                transformedColumnValue = columnValueString;
                            }

                            if (visibleSelectionFieldNameList.contains(columnLabel)) {
                                visibleRow.add(transformedColumnValue);
                            }

                            rowAsMap.put(columnLabel, rawColumnValue);
                        }
                            ...
                            if (fileAttachmentMap.containsKey(columnLabel)) {
                                ...
                                switch (exportAttachment.getSource().getSettingsMode()) {
                                    ...
                                    case FILE_RELATIVE_LOCAL_PATH: {
                                        String filePath =
                                                exportAttachment.getSource().getPrefixForFileRelativeLocalPath() + File.separator + String.valueOf(columnValue);
                                        List<AbcTuple3<String, String, String>> tuples = prepareAttachment(filePath,
                                                rowAsMap,
                                                exportAttachment,
                                                fileAttachmentsDirectoryPath);
                    ...
                    if (batchOfRows.size() >= finalBatchSizeOfFetch
                        ...
                        try {
                            ...
                            LOGGER.error("failed to append to file:{}", dataFilePath, e);
    ...
    private List<AbcTuple3<String, String, String>> prepareAttachment(
        ...
        if (Boolean.TRUE.equals(exportAttachment.getSource().getMayContainMultipleItemsInOneField())) {
            ...
            switch (exportAttachment.getNamingPolicy()) {
                ...
                case RANDOM: {
                    AbcTuple3<String, String, String> tuple =
                            new AbcTuple3<>(columnValue.trim(),
                                    directoryPath.toString(),
                                    UUID.randomUUID().toString());
                    result.add(tuple);
                }
