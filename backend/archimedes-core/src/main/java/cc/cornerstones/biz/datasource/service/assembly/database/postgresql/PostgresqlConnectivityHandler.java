...
@Component
public class PostgresqlConnectivityHandler implements ConnectivityHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(PostgresqlConnectivityHandler.class);

    private static final String JDBC_DRIVER_PREFIX = "jdbc:postgresql://";
    ...
    private static DruidDataSource getDataSource(PostgresqlConnectionProfile objectiveConnectionProfile) {
        String hashedKey =
    ...
    public DataSourceConnection createConnection(
        ...
        PostgresqlConnectionProfile objectiveConnectionProfile = null;
        try {
            objectiveConnectionProfile = JSONObject.toJavaObject(connectionProfile,
                    PostgresqlConnectionProfile.class);
        } catch (Exception e) {
            LOGGER.error("fail to transform postgresql connection profile:{}", connectionProfile, e);
            throw new AbcResourceConflictException("unexpected postgresql connection profile");
        }
        ...
        try {
            ...
            PostgresqlConnection objectiveConnection = new PostgresqlConnection();
            objectiveConnection.setConnection(connection);
            return objectiveConnection;
        } catch (SQLException e) {
            LOGGER.error("fail to create postgresql connection for {}", connectionProfile, e);
            throw new AbcResourceConflictException("fail to create postgresql connection");
        } finally {
        ...
        try {
            ...
            LOGGER.error("fail to create postgresql connection for {}", url, e);
            throw new AbcResourceConflictException("fail to create postgresql connection");
        } catch (SQLException e) {
            LOGGER.error("fail to create postgresql connection for {}", url, e);
            throw new AbcResourceConflictException("fail to create postgresql connection");
    ...
    public void closeConnection(
        ...
        try {
            ...
            LOGGER.warn("fail to check postgresql connection status", e);
        }

        try {
            objectiveConnection.getConnection().close();
        } catch (SQLException e) {
            LOGGER.error("fail to close postgresql connection", e);

            throw new AbcResourceConflictException("fail to close postgresql connection");
        }
    }
